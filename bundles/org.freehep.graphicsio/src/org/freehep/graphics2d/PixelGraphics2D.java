...
public class PixelGraphics2D extends AbstractVectorGraphics {
    ...
    static class SymbolBlitKey extends RenderingHints.Key {
        ...
        public String toString() {
            return "Symbol Blitting enable key"; //$NON-NLS-1$
        }
    }
    ...
    private static Map<WebColor, Image[][][]> symbols;
    ...
    static {
        ...
        displayLocal = false;
        try {
            Class<?> clazz = Class.forName("sun.awt.X11GraphicsEnvironment"); //$NON-NLS-1$
            displayX11 = true;
            Method method = clazz
                    .getMethod("isDisplayLocal", (Class<?>[]) null); //$NON-NLS-1$
    ...
    public String toString() {
        return "PixelGraphics2D[" + hostGraphics.toString() + "]"; //$NON-NLS-1$ //$NON-NLS-2$
    }
