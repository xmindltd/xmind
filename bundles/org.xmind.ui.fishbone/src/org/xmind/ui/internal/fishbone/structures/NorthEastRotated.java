...
//        PrecisionRectangle hTopicBounds = hTopicIns.getBounds(origin);
        ...
        for (int i = 0; i < calloutBranches.size(); i++) {
            ...
            double distTop = (-tan
                    * (hChildBounds.getBottomRight().x - topLineX) + topLineY - hChildBounds
                        .getBottomRight().y) / Math.sqrt(tan * tan + 1);
            double distBottom = (-tan
                    * (hChildBounds.getBottomRight().x - bottomLineX)
                    + bottomLineY - hChildBounds.getBottomRight().y)
                    / Math.sqrt(tan * tan + 1);
