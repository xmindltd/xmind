...
import org.xmind.org.freehep.util.io.FinishableOutputStream;
...
public class PDFStream extends PDFDictionary implements PDFConstants {
    ...
    void close() throws IOException {
        ...
        if (gStates > 0) {
            System.err
                    .println("PDFStream: unbalanced saves()/restores(), too many saves: "
                            + gStates);
        }
    }
    ...
    public void restore() throws IOException {
        if (gStates <= 0) {
            System.err
                    .println("PDFStream: unbalanced saves()/restores(), too many restores");
        }
        gStates--;
        println("Q");
    }
    ...
    public void matrix(AffineTransform xform) throws IOException {
        matrix(xform.getScaleX(), xform.getShearY(), xform.getShearX(),
                xform.getScaleY(), xform.getTranslateX(), xform.getTranslateY());
    }
    ...
    public void miterLimit(double limit) throws IOException {
     ...
     *
     * @param encode
     *            {@link ImageConstants#ZLIB} or {@link ImageConstants#JPG}
     * @return {@link #decodeFilters(String[])}
     */
    private PDFName[] getFilterName(String encode) {
        if (ImageConstants.ZLIB.equals(encode)) {
            return decodeFilters(new String[] { ImageConstants.ENCODING_FLATE,
                    ImageConstants.ENCODING_ASCII85 });
        }

        if (ImageConstants.JPG.equals(encode)) {
            return decodeFilters(new String[] { ImageConstants.ENCODING_DCT,
                    ImageConstants.ENCODING_ASCII85 });
        }

        throw new IllegalArgumentException("unknown image encoding " + encode
                + " for PDFStream");
    }
     ...
     *            Image to write
     * @param bkg
     *            Background color, null for transparent image
     * @param encode
     *            {@link org.xmind.org.freehep.graphicsio.ImageConstants#ZLIB} or
     *            {@link org.xmind.org.freehep.graphicsio.ImageConstants#JPG}
     * @throws java.io.IOException
     *             thrown by ImageBytes
     */
    public void image(RenderedImage image, Color bkg, String encode)
            throws IOException {

        ImageBytes bytes = new ImageBytes(image, bkg, encode,
                ImageConstants.COLOR_MODEL_RGB);
    ...
    public void imageMask(RenderedImage image, String encode)
        ...
        ImageBytes bytes = new ImageBytes(image, null, encode,
                ImageConstants.COLOR_MODEL_A);
    ...
    public void inlineImage(RenderedImage image, Color bkg, String encode)
        ...
        ImageBytes bytes = new ImageBytes(image, bkg, ImageConstants.JPG,
                ImageConstants.COLOR_MODEL_RGB);
