...
public abstract class AbstractVectorGraphicsIO extends VectorGraphicsIO {
    ...
    public static final String EMIT_WARNINGS = rootKey + ".EMIT_WARNINGS"; //$NON-NLS-1$

    public static final String TEXT_AS_SHAPES = rootKey + "." //$NON-NLS-1$
            + FontConstants.TEXT_AS_SHAPES;

    public static final String EMIT_ERRORS = rootKey + ".EMIT_ERRORS"; //$NON-NLS-1$

    public static final String CLIP = rootKey + ".CLIP"; //$NON-NLS-1$
    ...
    protected AbstractVectorGraphicsIO(Dimension size,
        ...
        super.setFont(new Font("Dialog", Font.PLAIN, 12)); //$NON-NLS-1$
    ...
    public void drawString(String string, double x, double y) {
        ...
        if (isProperty(TEXT_AS_SHAPES)) {
            ...
            if (fontName.equals("Symbol") || fontName.equals("ZapfDingbats")) { //$NON-NLS-1$ //$NON-NLS-2$
    ...
    public void drawString(AttributedCharacterIterator iterator, float x,
        ...
        if (isProperty(TEXT_AS_SHAPES)) {
            ...
            Map<? extends Attribute, Object> attributes = FontUtilities
                    .getAttributes(font);
            ...
            for (char c = iterator.first(); c != AttributedCharacterIterator.DONE; c = iterator
                ...
                if (attributes.equals(iterator.getAttributes())) {
                    ...
                    if (sb.length() > 0) {
                        ...
                        TextLayout tl = new TextLayout(sb.toString(),
                                attributes, getFontRenderContext());

                        // calculate real width
                        x = x
                                + Math.max(tl.getAdvance(), (float) tl
                                        .getBounds().getWidth());
                    }
    ...
    protected void writeSetTransform(AffineTransform transform)
        ...
        try {
            ...
            // ignored...
            System.err.println("Warning: (ignored) Could not invert matrix: " //$NON-NLS-1$
                    + oldTransform);
        }
    }
    ...
    protected void writeWidth(float width) throws IOException {
        writeWarning(getClass() + ": writeWidth() not implemented."); //$NON-NLS-1$
    }
    ...
    protected void writeCap(int cap) throws IOException {
        writeWarning(getClass() + ": writeCap() not implemented."); //$NON-NLS-1$
    }
    ...
    protected void writeJoin(int join) throws IOException {
        writeWarning(getClass() + ": writeJoin() not implemented."); //$NON-NLS-1$
    }
    ...
    protected void writeMiterLimit(float limit) throws IOException {
        writeWarning(getClass() + ": writeMiterLimit() not implemented."); //$NON-NLS-1$
    }
    ...
    protected void writeDash(double[] dash, double phase) throws IOException {
        writeWarning(getClass() + ": writeDash() not implemented."); //$NON-NLS-1$
    }
     ...
     * FontRenderContext with anti-aliasing and uses fractional metrics.
     *
